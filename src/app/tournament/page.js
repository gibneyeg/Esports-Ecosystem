...
export default function Tournaments() {
  ...
  const renderTournamentCard = (tournament) => {
    ...
    const registrationOpen = new Date() < new Date(tournament.registrationCloseDate);
  
    return (
      <div key={tournament.id} className="flex-none bg-white p-4 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 w-72 relative overflow-hidden group">
        <div className="absolute inset-0 bg-gradient-to-t from-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
  
        {!isCompleted && registrationOpen && (
          <div className="absolute top-2 right-2 bg-orange-500 text-white text-xs font-medium px-2 py-1 rounded-full">
            {getTimeUntilClose(tournament)}
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            ...
            <div className="pt-1">
              <div className="text-xs text-gray-600">Registration Deadline</div>
          ...
          <div className="space-y-2">
            <Link
              ...
              className={`w-full py-2 px-4 text-white text-sm text-center font-medium rounded-lg transition-colors ${
                isCompleted
